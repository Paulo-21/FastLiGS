...
pub fn solve(af : &ArgumentationFramework) -> Vec<usize> {
    ...
    while has_changed {
        ...
        for i in &label_in_new {
            for argument in af.af_attackee.get_unchecked(*i) {
                if !*label_out.get_unchecked(*argument as usize) {
                    label_out[*argument as usize] = true;
                    suspect_in.push(af.af_attackee.get_unchecked(*argument as usize));
                }
            }
        }
        for el in &suspect_in {
            for index_of_suspect in &**el {
                if !*label_out.get_unchecked(*index_of_suspect as usize) && all_attackers_are_out(af, &label_out, *index_of_suspect as usize)  {
...
fn all_attackers_are_out(af : &ArgumentationFramework, labelling : &[bool], index : usize) -> bool {
    unsafe {
        for attacker in af.af_attacker.get_unchecked(index) {
            if !*labelling.get_unchecked((*attacker) as usize) {
                return false;
            }
        }
    }
